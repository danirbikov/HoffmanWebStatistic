        ...
        public static void AddAllJsonFiles(ApplicationDbContext _dbContext)
        {
            ...
            try
            {
                ...
                foreach (string IP in standIPs)
                    ...
                    try
                    {
                        foreach (var file in Directory.GetFileSystemEntries(sourceFilePath, "*.json", SearchOption.AllDirectories).Where(k => DateTime.Now.Subtract(new FileInfo(k).CreationTime).Days <= 100))
